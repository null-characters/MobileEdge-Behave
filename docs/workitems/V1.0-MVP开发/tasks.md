...
| T-08 | 实现 StateEventRepository | Domain/Repository | T-07 | 1. saveEvent(event) 保存成功<br>2. getEventsByDate(date) 返回当日事件列表<br>3. 使用 Flow 或 suspend 函数 | 待开始 |
...
| 编号 | 任务描述 | 影响模块 | 依赖 | 测试要点 | 状态 |
|------|----------|----------|------|----------|------|
| T-11 | 实现统计界面 | Presentation/UI | T-09 | 1. 显示当日数据：在岗时长、离岗时长、离岗次数<br>2. 显示当前状态（在岗/离岗）<br>3. 显示开始/停止按钮<br>4. 数据实时更新 | 待开始 |
| T-12 | 实现 CSV 导出功能 | Presentation/Export | T-07 | 1. 导出格式：timestamp,from_state,to_state,confidence<br>2. 文件保存到 Downloads 目录<br>3. 导出成功提示 | 待开始 |
...
| 编号 | 任务描述 | 影响模块 | 依赖 | 测试要点 | 状态 |
|------|----------|----------|------|----------|------|
| T-15 | 性能优化 | 全局 | T-10, T-11, T-12 | 1. 内存占用 < 300MB<br>2. 功耗 < 5W<br>3. 设备温度 < 45°C<br>4. 连续运行 8 小时无性能劣化 | 待开始 |
